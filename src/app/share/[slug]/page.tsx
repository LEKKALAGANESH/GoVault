...
export default async function PublicTripSharePage({ params }: PageProps) {
  ...
  // Fetch trip data by slug (no auth required for public view)
  ...
  // Check if logged-in user is a trip member (owner or linked traveler)
  const isOwner = user?.id === trip.owner_id;
  const isTripMember = isOwner || (
    user && travelers?.some((t: Traveler) => t.user_id === user.id)
  );

  // If user is a trip member, they can see private info
  const canViewPrivateInfo = !!isTripMember;
  const loginUrl = `/login?redirect=/share/${slug}`;
  ...
  // Use defaults for Thailand trips when no data exists
  ...
  const tripStats: TripStats = {
    flights: { total: flights.length, confirmed: flights.filter((f) => f.status === "CONFIRMED").length },
    hotels: { total: hotels.length, confirmed: hotels.filter((h) => h.status === "CONFIRMED").length },
    todos: { total: finalTodos.length, completed: finalTodos.filter((t) => t.completed).length },
    packing: { total: finalPackingItems.length, checked: finalPackingItems.filter((p) => p.checked).length },
    documents: 0, // Hidden in public view
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-50">
      {/* Public View Banner */}
      <div className="bg-gradient-to-r from-teal-500 to-cyan-500 text-white text-center py-2.5 text-sm font-medium">
        You&apos;re viewing a shared trip itinerary
      </div>

      {/* Sticky Navigation - Same order as main page */}
      ...
      <section id="itinerary" className="py-12">
        <div className="max-w-4xl mx-auto px-4">
          ...
          {finalDays.length > 0 ? (
            ...
            <EmptySection message="No itinerary days planned yet" />
          )}
        </div>
      </section>
      ...
      <section id="flights" className="py-12">
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="font-playfair text-2xl md:text-3xl text-center text-white mb-8">
            ✈️ Flight <span className="text-sky-200">Bookings</span>
          </h2>
          {flights.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {flights.map((flight, idx) => (
                <FlightCard
                  ...
                  tripId={tripId}
                  isOwner={false}
                  hidePrivateInfo={!canViewPrivateInfo}
                  loginUrl={loginUrl}
                />
              ))}
            </div>
      ...
      <section id="hotels" className="py-12">
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="font-playfair text-2xl md:text-3xl text-center text-white mb-8">
            🏨 Hotel <span className="text-cyan-200">Stays</span>
          </h2>
          {hotels.length > 0 ? (
      ...
      <section id="todos">
        {finalTodos.length > 0 ? (
          <TripTodos todos={finalTodos} tripId={tripId} editable={false} />
        ) : (
          <div className="py-12 bg-gradient-to-br from-orange-500 via-orange-400 to-amber-400">
            <div className="max-w-4xl mx-auto px-4">
              <h2 className="font-playfair text-2xl md:text-3xl text-center text-white mb-8">
                ✅ Things <span className="text-orange-100">To Do</span>
              </h2>
              <EmptySection message="No to-do items yet" light />
            </div>
          </div>
        )}
      </section>
      ...
      <section id="survival">
        {finalSurvivalTips.length > 0 ? (
          ...
          <div className="py-12 bg-gradient-to-br from-emerald-500 via-teal-600 to-cyan-600">
            <div className="max-w-6xl mx-auto px-4">
              <h2 className="font-playfair text-2xl md:text-3xl text-center text-white mb-8">
                🎒 Travel <span className="text-emerald-200">Guide</span>
              </h2>
      ...
      <section id="phrases">
        {finalPhrases.length > 0 ? (
          <PhrasesSection phrases={finalPhrases} tripId={tripId} editable={false} />
        ) : (
          <div className="py-12 bg-gradient-to-br from-pink-500 via-rose-500 to-orange-400">
            <div className="max-w-6xl mx-auto px-4">
              <h2 className="font-playfair text-2xl md:text-3xl text-center text-white mb-8">
                🗣️ Essential <span className="text-pink-200">Phrases</span>
              </h2>
      ...
      <section id="packing">
        {finalPackingItems.length > 0 ? (
          <PackingChecklist items={finalPackingItems} tripId={tripId} editable={false} />
        ) : (
          <div className="py-12 bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50">
            <div className="max-w-6xl mx-auto px-4">
